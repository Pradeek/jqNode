			...
			if(method == regexRoute.method) {
				if(pathName.match(regexRoute.regex)) {
					...
					for(var j = 0, length = regexRoute.variables.length; j < length; j++) {
						...
						urlData[urlVariable.name] = sections[urlVariable.index];
					}
					if(method === "POST") {
						_data = "";
						request.addListener("data", function(chunk) {
						...
						request.addListener("end", function() {
							regexRoute.callback(request, response, require('querystring').parse(_data), urlData);
						});
					} else {
						regexRoute.callback(request, response, urlData);
					} 
...
$.readFile = function(fileName, callback, callbackArgs) {
...
$.writeFile = function(fileName, contentType) {
	...
	$.readFile(fileName, function(data, args) {
		if(data == "") {
